...
class MessagePostMerger {
	...
	function SubstituteFieldNames ( $text, $useHtmlEntities = true ) {
		...
		if( strpos(	$text, '[_form_results_]') !== false ) {

			$text = str_ireplace( '[_form_results_]', $this->_FormatFormContents( $useHtmlEntities ), $text );
	...
	private function _PaymentFormSubstitutions ( &$needles, &$replacements ) {
		...
		$needles[] = '[_form_invoicetext_]';
		$needles[] = '[_form_price_]';
		$needles[] = '[_cart_total_]';
